      ...
      const HELP_ITEMS = [
        {
          ...
          short:
            "Tres gráficas: votos por partido 2018–2024, ganador por año y pastel de competencia del último año.",
          bullets: [
        ...
        },
        {
          ...
          short:
            "Muestra votos por partido del año elegido y resume ganador, votos y margen absoluto/%.",
          bullets: [
        ...
        },
        {
          ...
          short:
            "Serie histórica de participación (VOTOS/LN %) por año disponible.",
          bullets: [
        ...
        {
          id: "comp-margen-tiempo",
          title: "Margen en el tiempo (sección)",
          short: "Evolución del margen entre 1º y 2º lugar por año.",
          bullets: [
            "Detecta si la sección se volvió más competida o más segura.",
          ],
        },
        {
          id: "comp-swing",
          title: "Swing (sección)",
          short: "Cambio de votos/participación por partido entre elecciones.",
          bullets: ["Mide ganancia/pérdida relativa de cada partido."],
        },
        {
          id: "comp-h2h",
          title: "Head-to-Head PAN vs MORENA",
          short: "Ventaja en votos/% por año entre PAN y MORENA.",
          bullets: ["Línea con valores ± y ganador del duelo."],
        },
        {
          id: "comp-hhi",
          title: "Competitividad (HHI)",
          short: "Índice de concentración (∑ share²).",
          bullets: ["Bajo = competido; alto = concentrado."],
        },
        {
          id: "comp-vol",
          title: "Volatilidad (Pedersen)",
          short: "Suma de cambios absolutos en shares / 2.",
          bullets: ["Cuánta “movida” hubo entre elecciones."],
        },
        {
          id: "comp-proy",
          title: "Proyección simple",
          short: "Tendencia lineal por partido y pronóstico.",
          bullets: ["Exploratorio; con cautela."],
        },
      ];
      ...
      function openHelpModal() {
        ...
        if (!body.__built) {
          ...
          <details>
            ...
            ${
              it.bullets?.length
                ? `<ul style="margin:.2rem 0 .6rem 1rem">${it.bullets
                    .map((b) => `<li>${b}</li>`)
                    .join("")}</ul>`
                : ""
            }
      ...
      document.addEventListener("DOMContentLoaded", () => {
        ...
        AT_dockPanel(document.getElementById("hist-panel"));
      ...
      async function showSectionInfo(feat) {
        ...
        const box = document.getElementById("sec-info");
        const hdr = box?.querySelector(".hdr");
        if (hdr) hdr.textContent = `Sección ${sec}`;
        const elDf = document.getElementById("si-df");
        const elDl = document.getElementById("si-dl");
        const elLn = document.getElementById("si-ln");
        if (elDf) elDf.textContent = df;
        if (elDl) elDl.textContent = dl;
        if (elLn) elLn.textContent = ln;
      ...
      function getSeccionesFromFeatures(feats) {
        return [...new Set(
          (feats || [])
            .map((f) => f.properties?.SECCION)
      ...
      function getBaseMunKey() {
        return (
          __mini.selMun?.value ||
          window.AT_BASE_MUN ||
          (window.AT_UNIVERSE?.scope === "MUN" ? window.AT_UNIVERSE.key : null)
        );
      }
      function updateMiniDlDfOptions(munKey) {
        const raw = window.AT_DATA || {};
        const feats = raw.features || [];
        const filtered = munKey
          ? feats.filter(
              (f) =>
                toComp(f.properties?.[FIELD_KEYS.mun]) === toComp(munKey)
            )
          : feats;
        const dfVals = uniqueSorted(
          filtered.map((f) => f.properties?.[FIELD_KEYS.df])
        );
        const dlVals = uniqueSorted(
          filtered.map((f) => f.properties?.[FIELD_KEYS.dl])
        );
        const curDf = __mini.selDf?.value || "";
        const curDl = __mini.selDl?.value || "";
        fillSelect(__mini.selDf, dfVals);
        fillSelect(__mini.selDl, dlVals);
        if (curDf && dfVals.includes(curDf)) __mini.selDf.value = curDf;
        if (curDl && dlVals.includes(curDl)) __mini.selDl.value = curDl;
      }
      ...
      function miniExclusivity(which) {
        ...
        if (which === "ALL") {
          ...
          selDf.value = "";
          selDl.value = "";
          window.AT_BASE_MUN = null;
        }
        if (which === "MUN") {
          selDf.value = "";
          selDl.value = "";
          all.checked = false;
          window.AT_BASE_MUN = selMun.value || null;
        }
        if (which === "DF") {
          selMun.value = "";
          selDl.value = "";
        ...
        updateMiniDlDfOptions(getBaseMunKey());
        applyMiniUniverse();
      }
      function applyMiniUniverse() {
        ...
        const seccionesUni2 = getSeccionesFromFeatures(filtered2.features || []);
        window.seccionesUniverso = seccionesUni2;
        window.totalSeccionesUniverso = seccionesUni2.length;
        window.AT_UNIVERSE = u2;
        window.AT_SCOPE = u2.scope;
        window.AT_MUN_ID = u2.scope === "MUN" ? u2.key : null;
        if (typeof window.updateATScopeBadge === "function") {
          window.updateATScopeBadge(u2, seccionesUni2);
        }
        if (window.AT_CTX?.layer) {
          atMap.removeLayer(AT_CTX.layer);
        }
        const layer2 = L.geoJSON(filtered2, {
          style: (feat) => qaStyleForFeature(feat, "#7d0025"),
          onEachFeature: (feat, lyr) => {
            ...
            lyr.on("click", () => {
              if (typeof gotoSection === "function")
                gotoSection({ sec: p.SECCION, feature: feat });
            });
          },
        }).addTo(atMap);
        try {
      ...
      function initMiniSelector(raw, u) {
        ...
        if (u.scope === "MUN") {
          ...
          window.AT_BASE_MUN = String(u.key);
        }
        if (u.scope === "DF") {
      ...
      function gotoSection(item) {
        ...
        if (!feat) {
          const feats =
            ...
            [];
          feat = feats.find(
            (f) => normSec(f.properties?.SECCION) === normSec(sec)
          );
        }
        ...
        document
          .getElementById("btn-competitividad")
        ...
        // 3.1) Si el panel de historial está abierto, refresca gráficas
        const histPanel = document.getElementById("hist-panel");
        if (histPanel && histPanel.style.display !== "none") {
          requestAnimationFrame(() => renderHistorialCharts());
        }
      ...
      document.addEventListener("DOMContentLoaded", async () => {
        ...
        // ================================
        // UNIVERSO TERRITORIAL (AT-27-30)
        // ================================

        // 1️⃣ Leer selección desde portal (si no existe, default = Estado)
        const scope = localStorage.getItem('AT_SCOPE') || 'STATE'; // 'STATE' | 'MUN'
        const munId = localStorage.getItem('AT_MUN_ID');           // ej. '6'

        // 2️⃣ Clonar features originales (NUNCA mutar el geojson base)
        let featuresUniverso = [...raw.features];

        // 3️⃣ Filtrar por municipio si aplica
        if (scope === 'MUN' && munId) {
          featuresUniverso = featuresUniverso.filter(f =>
            Number(f.properties?.MUNICIPIO) === Number(munId)
          );
        }

        // 4️⃣ Extraer secciones del universo activo
        const seccionesUniverso = getSeccionesFromFeatures(featuresUniverso);
        window.seccionesUniverso = seccionesUniverso;
        window.totalSeccionesUniverso = seccionesUniverso.length;
        window.AT_UNIVERSE = u;
        window.AT_SCOPE = scope;
        window.AT_MUN_ID = munId;

        // 🔎 DEBUG OPCIONAL
        console.log('AT_SCOPE:', scope);
        console.log('AT_MUN_ID:', munId);
        console.log('Total secciones universo:', seccionesUniverso.length);
        ...
        function updateATScopeBadge(uLocal, seccionesLocal) {
          const el = document.getElementById("atScopeBadge");
          if(!el) return;

          const uEff = uLocal || window.AT_UNIVERSE || {};
          const scopeLocal = String(uEff.scope || window.AT_SCOPE || scope || "").toUpperCase();
          const munIdLocal = uEff.key ?? window.AT_MUN_ID ?? munId ?? "—";
          const total =
            seccionesLocal?.length ??
            window.seccionesUniverso?.length ??
            window.totalSeccionesUniverso ??
            0;

          // Si ya tienes un label calculado en tu objeto universo, úsalo
          let munName = null;
          if (scopeLocal === "MUN" && munIdLocal && munIdLocal !== "—") {
            munName = getMunNameFromCatalog(munIdLocal);
            if (!munName || munName === String(munIdLocal)) {
              const hit = raw.features?.find(
                (f) => String(f.properties?.[munCodeKey]) === String(munIdLocal)
              );
              munName = hit?.properties?.[MUN_NAME_KEY] ?? munName;
            }
          }
          let label = uEff.label || scopeLocal || "—";
          if (scopeLocal === "MUN") {
            label = munName || uEff.label || String(munIdLocal);
            label = String(label).replace(/^Municipio\s+/i, "");
          }

          el.innerHTML = `UNIVERSO: <strong>${label}</strong> · Secciones: <strong>${total}</strong>`;
        }
        window.updateATScopeBadge = updateATScopeBadge;
        updateATScopeBadge(u, seccionesUniverso);
        ...
        const layer = L.geoJSON(filtered, {
          style: (feat) => qaStyleForFeature(feat, "#000000"),
          onEachFeature: (feat, lyr) => {
            const p = feat.properties || {};
            const muni = p[FIELD_KEYS.mun] ?? "—";
            const dl = p[FIELD_KEYS.dl] ?? "—";
        ...
        window.AT_CTX = { universe: u, paths, map: atMap, layer, baseColor: "#000000" };
      ...
      function detectSchemaFrom(data) {
        ...
        const row =
          slot[String(years[years.length - 1])] || slot[String(years[0])] || {};
        const parties =
          data?.meta?.parties && data.meta.parties.length
            ? data.meta.parties
      ...
      function openHistorialUI() {
        /* abrir panel/modal con charts */
      }
      function openProyeccionUI() {
        /* ... */
      }
      async function paintByMargin({ puesto, year }) {
        /* ... usando universe/sectionMetrics */
      }
      ...
      function winnerOf(votes) {
        ...
        return { party: best, votes: max, total: tot };
      }
      ...
      (function wireHistPanel() {
        ...
        hdr.addEventListener("mousedown", (e) => {
          if (AT_isDockedPanel(box)) return;
          if (e.target.closest(".btn-close") || e.target.tagName === "SELECT")
            return;
          e.preventDefault();
          e.stopPropagation();
          const r = box.getBoundingClientRect();
      ...
      function getActiveSectionSec() {
        const hl = window.__SEC_HL;
        if (hl?.getLayers) {
          const lyr = hl.getLayers()[0];
          return lyr?.feature?.properties?.SECCION ?? null;
        }
        const t = document.getElementById("si-sec")?.textContent;
        return t && t !== "—" ? t : null;
      }

      // Compat no-op: por si quedó alguna llamada vieja
      function runTool() {
        /* eliminado: barra vertical */
      }
      ...
      // ====== Charts ======
      let __HIST_CHART_BAR = null,
        __HIST_CHART_WIN = null,
        __HIST_CHART_PIE = null,
        __HIST_RENDER_TOKEN = 0;
      function destroyHistCharts() {
        try {
          __HIST_CHART_BAR?.destroy();
        } catch (_) {}
        try {
          __HIST_CHART_WIN?.destroy();
        } catch (_) {}
        try {
          __HIST_CHART_PIE?.destroy();
        } catch (_) {}
        __HIST_CHART_BAR = __HIST_CHART_WIN = __HIST_CHART_PIE = null;
      }

      async function renderHistorialCharts() {
        const renderToken = ++__HIST_RENDER_TOKEN;
        const panel = document.getElementById("hist-panel");
        if (!panel || panel.style.display === "none") return;
        const puesto = document.getElementById("hist-puesto").value || "P";
        const sec = getActiveSectionSec();
        if (!sec) {
          alert("Selecciona una sección en el mapa.");
          return;
        }
        document.getElementById("hist-sec").textContent = sec;

        const data = await loadPuesto(puesto);
        if (renderToken !== __HIST_RENDER_TOKEN) return;
        const slot = findSecSlot(data, sec);
        if (!slot) {
          destroyHistCharts();
          console.warn("[Hist] sin datos", sec, puesto);
          return;
        }

        const isSexenal = ["P", "S", "G"].includes(puesto);
        const expected = isSexenal ? [2018, 2024] : [2018, 2021, 2024];
        const hasYearKey = (y) =>
          slot[String(y)] != null ||
          slot[String(y).slice(2)] != null ||
          slot[String(+String(y).slice(2))] != null;
        let years = expected.filter(hasYearKey);
        if (!years.length) {
          years = Object.keys(slot)
            .filter((k) => /^\d{2}$|^\d{4}$/.test(k))
            .map((k) => (k.length === 2 ? 2000 + Number(k) : Number(k)))
            .filter((y) => Number.isFinite(y))
            .sort();
        }
        const getYearRow = (y) =>
          slot[String(y)] ??
          slot[String(y).slice(2)] ??
          slot[String(+String(y).slice(2))] ??
          null;
        const rows = years.map((y) => getYearRow(y) || {});
        const hasPositiveValue = (arr) =>
          Array.isArray(arr) && arr.some((v) => Number(v || 0) > 0);

        destroyHistCharts();

        const barCanvas = document.getElementById("hist-bar");
        const winCanvas = document.getElementById("hist-win");
        const pieCanvas = document.getElementById("hist-pie");
        if (!barCanvas || !winCanvas || !pieCanvas) {
          console.warn("[Hist] faltan canvases para renderizar");
          return;
        }
        const barCtx = barCanvas.getContext("2d");
        const winCtx = winCanvas.getContext("2d");
        const pieCtx = pieCanvas.getContext("2d");
        if (!barCtx || !winCtx || !pieCtx) {
          console.warn("[Hist] no se pudieron obtener contexts 2D");
          return;
        }

        // 1) Historial (barras agrupadas)
        const barLabels = years.map(String);
        const barDS = CORE_PARTIES.map((p) => ({
          label: p,
          data: rows.map((r) => asInt0(r?.[p])),
          backgroundColor: PARTY_COLORS[p] || "#90a4ae",
        }));
        __HIST_CHART_BAR = new Chart(
          barCtx,
          {
            type: "bar",
            data: { labels: barLabels, datasets: barDS },
            options: {
              responsive: true,
        ...
        years.forEach((y, i) => {
          const r = rows[i] || {};
          const v = {};
          CORE_PARTIES.forEach((p) => (v[p] = asInt0(r[p])));
          const w = winnerOf(v);
          winLabels.push(`${y} • ${w.party || "—"}`);
          winData.push(asInt0(w.votes));
          winColors.push(PARTY_COLORS[w.party] || "#90a4ae");
        });
        __HIST_CHART_WIN = new Chart(
          winCtx,
          {
            type: "bar",
            data: {
              labels: winLabels,
              datasets: [
                { label: "Votos", data: winData, backgroundColor: winColors },
              ],
            },
            options: {
              responsive: true,
        ...
        let pieLabels = CORE_PARTIES.filter((p) => asInt0(rLast[p]) > 0);
        if (!pieLabels.length) pieLabels = [...CORE_PARTIES];
        ...
        requestAnimationFrame(() => {
          __HIST_CHART_BAR?.resize();
          __HIST_CHART_WIN?.resize();
          __HIST_CHART_PIE?.resize();
        });
        if (document.getElementById("hist-qa")?.style.display !== "none")
          renderHistQA();
      }
      ...
      function openHistorialUI() {
        ...
        if (!sel.__wired) {
          sel.addEventListener("change", renderHistorialCharts);
          sel.__wired = true;
        }
        requestAnimationFrame(() => renderHistorialCharts());
      }
      ...
      function getHistContext() {
        const sec =
          typeof getActiveSectionSec === "function"
            ? getActiveSectionSec()
            : null;
      ...
      function fmt(n) {
        return n == null || isNaN(n) ? "—" : Number(n).toLocaleString("es-MX");
      }
      function fmtPct(x) {
      ...
      async function getPuestoData(p) {
        ...
        const paths = JSON.parse(localStorage.getItem("AT_PATHS") || "{}");
        const url =
          (paths.electoral && paths.electoral[p]) || `../data/electoral/${p}.json`;
        const res = await fetch(url);
      ...
      async function renderHistQA() {
        ...
        if (!sec) {
          ctx.textContent = "Selecciona una sección en el mapa.";
          return;
        }

        const route = `../data/electoral/${puesto}.json → secciones["${String(
          sec
        )}"]["${String(year)}"]`;
        ctx.innerHTML = `
    ...
    `;
        ...
        if (!row) {
          tbl.innerHTML =
            "<tr><td>No hay datos para esta combinación.</td></tr>";
          warn.textContent = "";
          return;
        }
      ...
      <tbody>
        ...
        <!-- <tr><td>OTROS = VOTOS - Total partidos</td><td class="${
          otros !== 0 ? "bad-cell" : ""
        }">${fmt(otros)}</td></tr> -->
        <!-- <tr><td>Participación</td><td>${fmtPct(partPct)}</td></tr> -->
        <!-- <tr><td>Ganador</td><td>${ganador}</td></tr> -->
        <!-- <tr><td>Margen</td><td>${fmt(margen)} (${fmtPct(
          margenPct
        )})</td></tr>// -->
      </tbody>
    `;
      ...
      (function wireHistQA() {
        ...
        selP?.addEventListener("change", () => {
          if (qa.style.display !== "none") renderHistQA();
        });
        selY?.addEventListener("change", () => {
          if (qa.style.display !== "none") renderHistQA();
        });

        const btnClose = document.getElementById("qa-close");
        if (btnClose) {
      ...
      function qaStatusKey(sec, puesto, year, source) {
        const s = String(sec).replace(/\D+/g, "");
        return `QA_STATUS:${s}|${puesto}|${year}|${source}`;
      }
      function saveQAStatus(sec, puesto, year, source, status) {
        const key = qaStatusKey(sec, puesto, year, source);
        localStorage.setItem(key, status);
        const lastKey = `QA_STATUS_LAST:${String(sec).replace(/\D+/g, "")}`;
        localStorage.setItem(lastKey, JSON.stringify({ source, status }));
      }
      function getLastQAStatus(sec) {
        const lastKey = `QA_STATUS_LAST:${String(sec).replace(/\D+/g, "")}`;
        try {
      ...
      function qaStyleForFeature(feat, baseColor) {
        const p = feat?.properties || {};
        const sec = p.SECCION ?? p.SECCION_ID ?? p.SEC ?? null;
        const last = sec ? getLastQAStatus(sec) : null;
        const st = last?.status;
        if (st === "ok")
          return { color: "#2e7d32", weight: 1.6, fillOpacity: 0.15 };
        if (st === "bad")
          return { color: "#c62828", weight: 1.6, fillOpacity: 0.15 };
        return { color: baseColor || "#000", weight: 1.2, fillOpacity: 0.15 };
      }
      function refreshQAStyleForSection(sec) {
        const layer = window.AT_CTX?.layer;
        if (!layer?.eachLayer) return;
        const baseColor = window.AT_CTX?.baseColor || "#000";
        layer.eachLayer((lyr) => {
          const f = lyr.feature;
          if (normSec(f?.properties?.SECCION) === normSec(sec)) {
            lyr.setStyle(qaStyleForFeature(f, baseColor));
          }
        });
      }
      ...
      async function wireQAExternal(sec, puesto, year, rowInt) {
        ...
        document.getElementById("qa-url-ieeg").value =
          saved.urls?.ieeg ||
          "https://computosgto2024.ieeg.mx/#/gubernatura/entidad/votos-entidad/mapa";

        // Abrir URLs
        document.getElementById("qa-open-ine").onclick = () =>
          openIf(document.getElementById("qa-url-ine").value);
        document.getElementById("qa-open-ieeg").onclick = () =>
          openIf(document.getElementById("qa-url-ieeg").value);
        ...
        document.getElementById("qa-compare").onclick = () => {
          ...
          const sourceSel = document.getElementById("qa-source");
          const source = sourceSel?.value || "IEEG";
          // marcar campos vacíos o no numéricos como diferencia
          document
            .querySelectorAll("#qa-ext-table .qa-num")
            .forEach((inp) => inp.classList.remove("bad-input"));

          const intPAN = +rowInt.PAN || 0,
            intPRI = +rowInt.PRI || 0,
            intPVEM = +rowInt.PVEM || 0,
            intMORENA = +rowInt.MORENA || 0;
          const intVOT = +rowInt.VOTOS || 0,
            intLN = +rowInt.LN || 0;
          ...
          fields.forEach((f) => {
            ...
            const inp = document.querySelector(
              `#qa-ext-table .qa-num[data-f="${f}"]`
            );
            const mismatch = ext == null || Number.isNaN(ext) || ext !== intv;
            if (mismatch) {
              diffs.push(
                `• ${f}: App=${fmt(intv)} vs Ext=${
                  ext == null || Number.isNaN(ext) ? "—" : fmt(ext)
                }`
              );
          ...
          if (rowExt.VOTOS > 0) {
            if (gInt.ganador !== gExt.ganador)
              diffs.push(
                `• Ganador: App=${gInt.ganador} vs Ext=${gExt.ganador}`
              );
            // margen % tolerancia 0.1 p.p.
            const tol = 0.1;
            if (Math.abs(gInt.margenPct - gExt.margenPct) > tol)
              diffs.push(
                `• Margen%: App=${fmtPct(gInt.margenPct)} vs Ext=${fmtPct(
                  gExt.margenPct
                )}`
              );
            if (gInt.margen !== gExt.margen)
              diffs.push(
                `• Margen(v): App=${fmt(gInt.margen)} vs Ext=${fmt(
                  gExt.margen
                )}`
              );
          }
          ...
          if (diffs.length === 0) {
            ...
            res.classList.remove("bad-msg");
            res.classList.add("ok-msg");
          } else {
            res.textContent = "⚠ Diferencias:\n" + diffs.join("\n");
            res.classList.remove("ok-msg");
            res.classList.add("bad-msg");
          }
          saveQAStatus(sec, puesto, year, source, diffs.length === 0 ? "ok" : "bad");
          refreshQAStyleForSection(sec);
      ...
      function AT27_CHP_getPropsDeSeccion(seccionId) {
        ...
        const matchHL =
          propSec &&
          String(propSec).replace(/\D+/g, "").padStart(4, "0") === secKey;
        ...
        const hasVotes = Object.keys(out).some((k) =>
          /^(18|21|24)(A|DL|DF|G|S|P)_/.test(k)
        );
      ...
      function AT27_CHP_getOrCreatePanel() {
        ...
        panel.innerHTML = `
          <div id="AT27_CHP_hdr" style="cursor:move; user-select:none; background:#5b21b6; color:#fff; padding:10px 12px; display:flex; align-items:center; gap:10px;">
            ...
            <select id="AT27_CHP_selPuesto" title="Puesto" style="margin-left:auto; background:#fff; color:#111827; border-radius:8px; padding:4px 8px; border:none; outline:none;">
              ${Object.entries(AT27_CHP_PUESTOS)
        ...
        // Cerrar
        panel.querySelector("#AT27_CHP_btnClose").onclick = () =>
          panel.remove();
        ...
        panel.querySelector("#AT27_CHP_btnPNG").onclick = () => {
          ...
          a.href = url;
          a.download = "comparativa_historica_puesto.png";
          a.click();
        };
      ...
      function AT27_CHP_detectPartidos(props, puesto) {
        ...
        years.forEach((y) => {
          ...
          for (const k in props)
            if (k.startsWith(pref)) parts.add(k.slice(pref.length));
        });
          ...
          try {
            ...
            for (const r of arr) {
              ...
              for (const [k, v] of Object.entries(r)) {
                ...
                if (
                  /^(ID|Seccion|SECCION|seccion|ENTIDAD|MUNICIPIO|DISTRITO_)/.test(
                    k
                  )
                ) {
                  if (!(k in acc)) acc[k] = v;
                ...
                if (
                  (m = k.match(
                    /^(A|DL|DF|G|S|P)[\-_]?(2018|2021|2024)[\-_]?(.+)$/i
                  ))
                ) {
                  yy = m[2].slice(-2);
                  party = m[3].toUpperCase();
                ...
                if (
                  (m = k.match(
                    /^(2018|2021|2024)[\-_]?(A|DL|DF|G|S|P)[\-_]?(.+)$/i
                  ))
                ) {
                  yy = m[1].slice(-2);
                  party = m[3].toUpperCase();
                ...
                if (
                  (m = k.match(/^(?:V|VOTOS)?(2018|2021|2024)[\-_]?(.+)$/i))
                ) {
                  yy = m[1].slice(-2);
            ...
            /* ignora errores de red/formato para no romper el flujo */
          }
        }
      ...
      async function AT27_CHP_renderChart({ seccionId, puesto }) {
        ...
        const slot =
          dataPuesto?.secciones?.[secKey] ||
          dataPuesto?.secciones?.[String(Number(secKey))];
        if (!slot) {
          alert("No hay datos para la sección/puesto seleccionado.");
          return;
        }
        ...
        info.innerHTML = `Sección <b>${seccionId}</b> · Puesto <b>${
          AT27_CHP_PUESTOS[puesto] || puesto
        }</b>`;

        // Destruye instancia anterior si existe
        if (window.AT27_CHP_chart) {
          window.AT27_CHP_chart.destroy();
        }
        ...
        sel.onchange = () =>
          AT27_CHP_renderChart({ seccionId, puesto: sel.value });
        sel.value = puesto;
      }
      ...
      // ---------- Utils ----------
      function AT27_COMP_fmt(x) {
        return Number(x || 0).toLocaleString("es-MX");
      }
      function AT27_COMP_secKey(seccionId) {
        return String(seccionId ?? "").replace(/\D+/g, "");
      } // sin padStart
      ...
      async function AT27_COMP_getSlot(seccionId, puesto) {
        if (typeof loadPuesto !== "function")
          throw new Error("loadPuesto no está disponible");
        const js = await loadPuesto(puesto);
        const sec = AT27_COMP_secKey(seccionId);
        return (
          js?.secciones?.[sec] || js?.secciones?.[String(Number(sec))] || null
        );
      }
      ...
      function AT27_COMP_calcFromSlot(slot) {
        ...
        const votosTot = Number(
          row.VOTOS || entries.reduce((a, [, v]) => a + v, 0)
        );
        ...
        panel.innerHTML = `
          <div id="AT27_COMP_hdr" style="cursor:move; user-select:none; background:#0ea5e9; color:#fff; padding:10px 12px; display:flex; gap:10px; align-items:center;">
            ...
            <select id="AT27_COMP_selPuesto" style="margin-left:auto; background:#fff; color:#111827; border-radius:8px; padding:4px 8px; border:none; outline:none;" title="Puesto">
              ${Object.entries(AT27_COMP_PUESTOS)
                .map(([k, v]) => `<option value="${k}">${v}</option>`)
                .join("")}
            </select>
      ...
      function AT27_COMP_renderPanel({ seccionId, puesto, resultado }) {
        ...
        const { votosTot, top1, top2, margen, margenPct, status, ranking } =
          resultado;
        ...
        const filas = ranking
          ...
          <tr>
            ...
            <td style="padding:6px 8px; text-align:right;">${AT27_COMP_fmt(
              v
            )}</td>
          </tr>
        `
        ...
        body.innerHTML = `
          <div style="font-size:12px;color:#64748b;margin-bottom:8px;">
            Sección <b>${seccionId}</b> · Puesto <b>${
          AT27_COMP_PUESTOS[puesto] || puesto
        }</b> · Año <b>2024</b>
          </div>
          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-bottom:10px;">
            <div style="border:1px solid #f1f5f9;border-radius:12px;padding:10px;background:${AT27_COMP_color(
              status
            )}">
              <div style="font-size:12px;color:#334155;">Semáforo</div>
              <div style="font-weight:800;font-size:18px;margin-top:4px;">${status}</div>
              <div style="font-size:12px;color:#475569;margin-top:2px;">Margen ${AT27_COMP_fmt(
                margen
              )} (${margenPct.toFixed(1)} pp)</div>
            </div>
            <div style="border:1px solid #f1f5f9;border-radius:12px;padding:10px;">
              ...
              <div style="font-weight:800;font-size:18px;margin-top:4px;">${
                top1[0]
              } · ${AT27_COMP_fmt(top1[1])}</div>
              <div style="font-size:12px;color:#64748b;margin-top:2px;">2º: ${
                top2[0]
              } · ${AT27_COMP_fmt(top2[1])}</div>
            </div>
          </div>
          <div style="font-size:12px;color:#64748b;margin-bottom:8px;">
        ...
        sel.onchange = async () => {
          ...
          try {
            ...
            if (!slot) {
              alert("No hay datos para este puesto en la sección.");
              return;
            }
            ...
            console.error(err);
            alert("Error al recalcular competitividad.");
          }
        };
      }
      ...
      // --------- Utils ---------
      function AT27_TOP3_fmt(x) {
        return Number(x || 0).toLocaleString("es-MX");
      }
      function AT27_TOP3_secKey(seccionId) {
        return String(seccionId ?? "").replace(/\D+/g, "");
      } // sin padStart
      const AT27_TOP3_COLORS = {
        PAN: "#1E88E5",
        PRI: "#D32F2F",
        PRD: "#FBC02D",
        PVEM: "#50E991",
        ...
        panel.innerHTML = `
          <div id="AT27_TOP3_hdr" style="cursor:move; user-select:none; background:#10b981; color:#fff; padding:10px 12px; display:flex; gap:10px; align-items:center;">
            ...
            <select id="AT27_TOP3_selPuesto" style="margin-left:auto; background:#fff; color:#111827; border-radius:8px; padding:4px 8px; border:none; outline:none;" title="Puesto">
              ${Object.entries(AT27_TOP3_PUESTOS)
                .map(([k, v]) => `<option value="${k}">${v}</option>`)
                .join("")}
            </select>
      ...
      function AT27_TOP3_render({ seccionId, puesto, resultado }) {
        ...
        const { total, top, others, deltaVsPuntero, puntero, punterosVotos } =
          resultado;
        ...
        const rows = top
          .map((t, i) => {
            const ancho =
              punterosVotos > 0
                ? Math.round((t.votos / punterosVotos) * 100)
                : 0;
            const delta = deltaVsPuntero[t.partido] ?? 0;
            return `
            <div style="margin-bottom:10px;">
              <div style="display:flex; align-items:center; gap:8px;">
                <div style="font-weight:800; width:18px; text-align:center;">${
                  i + 1
                }</div>
                <div style="font-weight:700;">${t.partido}</div>
                <div style="margin-left:auto; font-variant-numeric: tabular-nums;">${AT27_TOP3_fmt(
                  t.votos
                )}</div>
                <div style="width:64px; text-align:right; color:#64748b; font-variant-numeric: tabular-nums;">${(
                  t.share * 100
                ).toFixed(1)}%</div>
              </div>
              <div style="display:flex; align-items:center; gap:8px; margin-top:6px;">
                <div style="flex:1; height:8px; background:#f1f5f9; border-radius:999px; overflow:hidden;">
                  <div style="width:${ancho}%; height:100%; background:${AT27_TOP3_colorFor(
              t.partido
            )};"></div>
                </div>
                <div style="min-width:120px; text-align:right; font-size:12px; color:${
                  delta === 0 ? "#10b981" : "#ef4444"
                };">
                  ${
                    delta === 0
                      ? "Puntero"
                      : "−" + AT27_TOP3_fmt(delta) + " vs puntero"
                  }
                </div>
              </div>
            </div>
        ...
        const otrosBloque =
          ...
          <div style="display:flex; justify-content:space-between; color:#64748b; font-size:12px; margin-top:4px;">
            ...
            <div>${AT27_TOP3_fmt(others)} (${
                total > 0 ? ((others / total) * 100).toFixed(1) : "0.0"
              }%)</div>
          </div>
        `
            : "";
        ...
        body.innerHTML = `
          <div style="font-size:12px;color:#64748b;margin-bottom:8px;">
            Sección <b>${seccionId}</b> · Puesto <b>${
          AT27_TOP3_PUESTOS[puesto] || puesto
        }</b> · Año <b>2024</b>
          </div>
          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-bottom:8px;">
            <div style="border:1px solid #f1f5f9;border-radius:12px;padding:10px;">
              <div style="font-size:12px;color:#334155;">Puntero</div>
              <div style="font-weight:800;font-size:18px;margin-top:2px;">${puntero} · ${AT27_TOP3_fmt(
          punterosVotos
        )}</div>
            </div>
            <div style="border:1px solid #f1f5f9;border-radius:12px;padding:10px;">
              <div style="font-size:12px;color:#334155;">Votos totales 2024</div>
              <div style="font-weight:800;font-size:18px;margin-top:2px;">${AT27_TOP3_fmt(
                total
              )}</div>
            </div>
          </div>
        ...
        sel.onchange = async () => {
          ...
          try {
            ...
            console.error(err);
            alert("Error al recalcular Top 3.");
          }
        };
      }
    ...
    */
      ...
      const AT27_HG_colorFor = (p) =>
        AT27_HG_COLORS[p] ||
        `hsl(${
          [...p].reduce((h, c) => (h * 31 + c.charCodeAt(0)) >>> 0, 0) % 360
        } 70% 45%)`;

      function AT27_HG_fmt(x) {
        return Number(x || 0).toLocaleString("es-MX");
      }
      ...
      function AT27_HG_getUniverseSecIds() {
        if (
          Array.isArray(window.AT_UNIVERSE_SECCIONES) &&
          window.AT_UNIVERSE_SECCIONES.length
        ) {
          return [
            ...new Set(
              window.AT_UNIVERSE_SECCIONES.map(AT27_HG_sec).filter(Boolean)
            ),
          ];
        }
        const feats =
      ...
      function AT27_HG_getOrCreatePanel() {
        ...
        <div id="AT27_HG_hdr" style="cursor:move; user-select:none; background:#7c3aed; color:#fff; padding:10px 12px; display:flex; gap:10px; align-items:center;">
          ...
          <select id="AT27_HG_selPuesto" style="margin-left:auto; background:#fff; color:#111827; border-radius:8px; padding:4px 8px; border:none; outline:none;">
            ${Object.entries(AT27_HG_PUESTOS)
              .map(([k, v]) => `<option value="${k}">${v}</option>`)
              .join("")}
          </select>
      ...
      function AT27_HG_render({ puesto, resumen, universeCount }) {
        ...
        const tiles = years
          ...
          <div style="border:1px solid #f1f5f9; border-radius:12px; padding:12px; display:grid; grid-template-columns: 1fr 1fr; gap:8px;">
            <div>
              ...
              <div style="font-size:12px; color:#64748b; margin-top:4px;">Total votos: <b>${AT27_HG_fmt(
                r.total
              )}</b></div>
            </div>
            <div>
              <div style="font-size:12px; color:#334155;">Ganador</div>
              <div style="font-weight:900; font-size:18px; color:${c};">${
              r.ganador
            }</div>
              <div style="font-size:12px; color:#64748b; margin-top:4px;">
                ${AT27_HG_fmt(r.votos)} (${(r.share * 100).toFixed(
              1
            )}%) · margen vs 2º: <b>${AT27_HG_fmt(r.margen)}</b>
              </div>
            </div>
          </div>
        `;
        ...
        body.innerHTML = `
        <div style="font-size:12px;color:#64748b;margin-bottom:8px;">
          Universo actual: <b>${universeCount}</b> secciones · Puesto <b>${
          AT27_HG_PUESTOS[puesto] || puesto
        }</b>
        </div>
        ...
        sel.onchange = async () => {
          ...
          try {
            ...
            AT27_HG_render({
              puesto: nuevo,
              resumen: res,
              universeCount: secIds.length,
            });
          } catch (e) {
            console.error(e);
            alert("No pude recalcular el historial.");
          }
        };
      }
      ...
      async function AT27_HG_ejecutar(puestoInicial = "A") {
        try {
          const secIds = AT27_HG_getUniverseSecIds();
          if (!secIds.length) {
            alert("No hay secciones en el universo actual.");
            return;
          }
      ...
      function AT27_GPA_yearsFor(puesto) {
        return ["G", "S", "P"].includes(puesto)
          ? ["2018", "2024"]
          : ["2018", "2021", "2024"];
      }
      ...
      function AT27_GPA_getUniverseSecIds() {
        if (
          ...
          return [
            ...new Set(
              window.AT_UNIVERSE_SECCIONES.map(AT27_GPA_sec).filter(Boolean)
            ),
          ];
        }
        const feats =
      ...
      function AT27_GPA_clearOverlay() {
        ...
        if (window.AT27_GPA_LAYER) {
          try {
            map.removeLayer(window.AT27_GPA_LAYER);
          } catch (e) {}
      ...
      function AT27_GPA_renderLegend(counts, universeSize) {
        const items = Object.entries(counts)
          ...
          .map(([p, c]) => {
            ...
            const pct =
              universeSize > 0 ? ((c / universeSize) * 100).toFixed(1) : "0.0";
            return `
            <div style="display:flex; align-items:center; gap:8px;">
        ...
        return (
          items ||
          `<div style="color:#64748b;">Sin datos para el universo/año/puesto seleccionados.</div>`
        );
      }
      ...
      function AT27_GPA_getOrCreatePanel() {
        ...
        <div id="AT27_GPA_hdr" style="cursor:move; user-select:none; background:#0ea5e9; color:#fff; padding:10px 12px; display:flex; gap:10px; align-items:center;">
          ...
          <select id="AT27_GPA_selPuesto" style="background:#fff; color:#111827; border-radius:8px; padding:4px 8px; border:none; outline:none;">
            ${Object.entries(AT27_GPA_PUESTOS)
              .map(([k, v]) => `<option value="${k}">${v}</option>`)
              .join("")}
          </select>
      ...
      `;
        ...
        // Cerrar
        panel.querySelector("#AT27_GPA_close").onclick = () => {
          AT27_GPA_clearOverlay();
          panel.remove();
        };
      ...
      async function AT27_GPA_update() {
        ...
        panel.querySelector("#AT27_GPA_uniCount").textContent = String(
          secIds.length
        );

        if (!secIds.length) {
          AT27_GPA_clearOverlay();
          panel.querySelector("#AT27_GPA_legend").innerHTML =
            "<i>Sin secciones</i>";
          return;
        }
        ...
        panel.querySelector("#AT27_GPA_legend").innerHTML =
          AT27_GPA_renderLegend(counts, secIds.length);
      }
      ...
      // ----- API pública -----
      async function AT27_GPA_ejecutar(
        puestoInicial = "A",
        yearInicial = "2024"
      ) {
        try {
          ...
          if (
            yearInicial &&
            [...selYear.options].some((o) => o.value === yearInicial)
          )
            selYear.value = yearInicial;

          // listeners
          selPuesto.onchange = () => {
            AT27_GPA_populateYears(selPuesto.value);
            AT27_GPA_update();
          };
          ...
          alert(
            "No pude calcular los ganadores por partido/año. Verifica que el universo y los JSON estén cargados."
          );
        }
      }
      ...
      function AT27_ABS_getUniverseSecIds() {
        if (
          ...
          return [
            ...new Set(
              window.AT_UNIVERSE_SECCIONES.map(AT27_ABS_sec).filter(Boolean)
            ),
          ];
        }
        const feats =
      ...
      function AT27_ABS_getOrCreatePanel() {
        ...
        <div id="AT27_ABS_hdr" style="cursor:move; user-select:none; background:#14b8a6; color:#fff; padding:10px 12px; display:flex; gap:10px; align-items:center;">
          ...
          <select id="AT27_ABS_selPuesto" style="margin-left:auto; background:#fff; color:#111827; border-radius:8px; padding:4px 8px; border:none; outline:none;">
            ${Object.entries(AT27_ABS_PUESTOS)
              .map(([k, v]) => `<option value="${k}">${v}</option>`)
              .join("")}
          </select>
      ...
      function AT27_ABS_render({ puesto, resumen, universeCount }) {
        ...
        const delta =
          isFinite(resumen[last].abst) && isFinite(resumen[first].abst)
            ? resumen[last].abst - resumen[first].abst
            : NaN;
        const arrow = !isFinite(delta)
          ? { txt: "—", col: "#64748b" }
          : delta > 0
          ? { txt: "↑", col: "#ef4444" }
          : delta < 0
        ...
        const tiles = years
          .map((y) => {
            const r = resumen[y];
            const bar =
              isFinite(resumen[first].abst) &&
              isFinite(r.abst) &&
              resumen[first].abst > 0
                ? Math.max(
                    2,
                    Math.min(
                      100,
                      Math.round((r.abst / resumen[first].abst) * 100)
                    )
                  )
                : 0;
            return `
          <div style="border:1px solid #f1f5f9;border-radius:12px;padding:10px;">
            ...
            <div style="font-size:12px;color:#64748b;margin-top:4px;">LN: <b>${AT27_ABS_fmtInt(
              r.ln
            )}</b> · Votos: <b>${AT27_ABS_fmtInt(r.votos)}</b></div>
            <div style="margin-top:8px;">
              <div style="font-size:12px;color:#334155;">Abstención</div>
              <div style="display:flex;align-items:baseline;gap:8px;">
                <div style="font-weight:900;font-size:20px;">${AT27_ABS_fmtPct(
                  r.abst
                )}</div>
                <div style="color:#64748b;font-size:12px;">(Participación: ${AT27_ABS_fmtPct(
                  r.part
                )})</div>
              </div>
        ...
        body.innerHTML = `
        <div style="font-size:12px;color:#64748b;margin-bottom:8px;">
          Universo: <b>${universeCount}</b> secciones · Puesto <b>${
          AT27_ABS_PUESTOS[puesto] || puesto
        }</b>
        </div>
        <div style="border:1px solid #e2e8f0;border-radius:12px;padding:10px;margin-bottom:10px;background:#f8fafc;">
          ...
          <b style="color:${arrow.col};">${arrow.txt} ${
          isFinite(delta) ? AT27_ABS_fmtPct(Math.abs(delta)) : "—"
        }</b>
        </div>
        <div style="display:grid; grid-template-columns: 1fr; gap:10px;">${tiles}</div>
      `;
      ...
      function AT27_HHI_getUniverseSecIds() {
        if (
          ...
          return [
            ...new Set(
              window.AT_UNIVERSE_SECCIONES.map(AT27_HHI_sec).filter(Boolean)
            ),
          ];
        }
        const feats =
      ...
      function AT27_HHI_aggregate2024(js, secIds) {
        ...
        let totalVotes = 0;
        for (const sec of secIds) {
          const row = js?.secciones?.[sec]?.["2024"];
          if (!row) continue;
          let sumRow = 0;
          for (const [k, v] of Object.entries(row)) {
            if (k === "VOTOS" || k === "LN") {
              sumRow += Number(k === "VOTOS" ? v || 0 : 0);
              continue;
            }
            const val = Number(v || 0);
            partyTotals[k.toUpperCase()] =
              (partyTotals[k.toUpperCase()] || 0) + val;
            sumRow += val;
          }
      ...
      function AT27_HHI_bucket(hhi) {
        ...
        if (hhi >= 0.45)
          return { label: "Concentración alta", color: "#ef4444" };
        if (hhi >= 0.3)
          return { label: "Concentración media", color: "#f59e0b" };
      ...
      function AT27_HHI_getOrCreatePanel() {
        ...
        <div id="AT27_HHI_hdr" style="cursor:move; user-select:none; background:#8b5cf6; color:#fff; padding:10px 12px; display:flex; gap:10px; align-items:center;">
          ...
          <select id="AT27_HHI_selPuesto" style="margin-left:auto; background:#fff; color:#111827; border-radius:8px; padding:4px 8px; border:none; outline:none;">
            ${Object.entries(AT27_HHI_PUESTOS)
              .map(([k, v]) => `<option value="${k}">${v}</option>`)
              .join("")}
          </select>
      ...
      function AT27_HHI_render({ puesto, resumen, universeCount }) {
        ...
        const rows = Object.entries(shares)
          ...
          .map(([p, s]) => {
            ...
            <div style="display:flex; align-items:center; gap:8px; margin-bottom:8px;">
              <span style="width:12px;height:12px;border-radius:3px;background:${AT27_HHI_colorFor(
                p
              )};border:1px solid rgba(0,0,0,.15)"></span>
              <div style="flex:1;">${p}</div>
              <div style="min-width:90px; text-align:right; font-variant-numeric: tabular-nums;">${(
                s * 100
              ).toFixed(1)}%</div>
            </div>
            <div style="height:8px;background:#f1f5f9;border-radius:999px;overflow:hidden;margin:-4px 0 6px 20px;">
              <div style="width:${w}%;height:100%;background:${AT27_HHI_colorFor(
              p
            )};"></div>
            </div>
          `;
        ...
        <div style="font-size:12px;color:#64748b;margin-bottom:8px;">
          Universo: <b>${universeCount}</b> secciones · Puesto <b>${
          AT27_HHI_PUESTOS[puesto] || puesto
        }</b>
        </div>
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-bottom:10px;">
          <div style="border:1px solid #f1f5f9;border-radius:12px;padding:10px;">
            ...
            <div style="font-weight:900;font-size:20px;">${
              isFinite(hhi) ? hhi.toFixed(3) : "—"
            }</div>
            <div style="font-size:12px;color:${bucket.color};margin-top:4px;">${
          bucket.label
        }</div>
          </div>
          <div style="border:1px solid #f1f5f9;border-radius:12px;padding:10px;">
            <div style="font-size:12px;color:#334155;">Votos totales 2024</div>
            <div style="font-weight:900;font-size:20px;">${AT27_ABS_fmtInt(
              totalVotes
            )}</div>
            <div style="font-size:12px;color:#64748b;margin-top:4px;">ENP (1/HHI): <b>${
              isFinite(enp) ? enp.toFixed(2) : "—"
            }</b></div>
          </div>
        </div>
        <div>${
          rows ||
          '<i style="color:#64748b;">Sin votos en el universo actual.</i>'
        }</div>
      `;
      }
...
// ---- inline block separator ----
      ...
      (function wireTopBar() {
        ...
        document
          .getElementById("btn-abrir-comp")
          ?.addEventListener("click", () => {
            const kind =
              document.getElementById("comp-metric")?.value || "ganador";
            if (typeof openComparativaUI === "function")
              openComparativaUI(kind);
          });
        ...
        document
          .getElementById("btn-comp-puesto")
          ?.addEventListener("click", () => {
            const sec =
              (typeof getActiveSectionSec === "function" &&
                getActiveSectionSec()) ||
            ...
            AT27_CHP_ejecutar(
              String(sec).replace(/\D+/g, "").padStart(4, "0"),
              "A"
            );
          });
...
// ---- inline block separator ----
      ...
      <div id="AT27_AT_help_body" style="padding:12px 14px; flex:1; overflow:auto; color:#111;">
        ...
        <p style="margin:10px 0 0; color:#64748b; font-size:12px;">
          <b>Tips:</b> usa el buscador para elegir sección; clic en el mapa abre detalles; puedes ajustar la opacidad de la capa.
        </p>
    ...
    `;
        ...
        // cerrar
        panel.querySelector("#AT27_AT_help_close").onclick = () => {
        ...
        (function drag(panelEl, handleEl) {
          ...
          window.addEventListener("mouseup", () => {
            dragging = false;
            document.body.style.userSelect = "";
            if (typeof clampToViewport === "function") clampToViewport(panelEl);
          });
      ...
      (function wireAT_Help() {
        const btn =
          ...
          document.getElementById("btn-at-ayuda");
